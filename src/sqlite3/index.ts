...
export default class OpfsDb {
    ...
    async open(name: string): Promise<SqliteResponse> {
        ...
        await promise;
        if (this.#sqlite !== null) {
            return await this.#sqlite("open", { filename: `file:${name}.sqlite3?vfs=opfs` });
        } else {
            return OpfsDb.#Fail();
        }
    }
    ...
    async exec(sql: string, rowMode: "array" | "object" = "array", bindings: Record<string, any> = {}): Promise<SqliteResponse> {
        if (this.#sqlite !== null) {
...
export class TaguetteDb extends OpfsDb {
    ...
    #qHighlightLookup: string = "SELECT id, snippet FROM highlights;";
    ...
    async getHighlights() {
        ...
        // @ts-expect-error
        const { result: { resultRows }, workerReceivedTime: t1,
            workerRespondTime: t2 } = await this.exec(this.#qHighlighAio, "object");
        const t3 = performance.now();
        return resultRows.map(({ id, snippet, tags, tagIds }: { id: number, snippet: string, tags: string, tagIds: string }) => ({
            id, snippet, tags: tags === null ? [] : tags.split(","), tagIds: tagIds === null ? [] : tagIds.split(",").map(Number)
        }));
    }
    async getTags() {
        // @ts-expect-error
        const { result: { resultRows } } = await this.exec(this.#qTags);
        return resultRows.reduce((acc: Record<number, string>, [tagId, tag]: [number, string]) => {
            acc[(tagId)] = tag;
            return acc;
        }, {});
    }
    async updateTagsForHighlight(hid: number, remove: number[], add: number[]) {
        ...
        if (add.length > 0) {
            ...
            const placeholders = add.map(key).map(k => `($hid,${k})`).join(',');
    ...
    async updateTagPaths(updateEntries: Array<[number, string]>) {
        ...
        bindings = updateEntries.reduce((acc: Record<string, any>, [tagId, tagPath]: [number, string], i: number) => {
            acc[`$e${i}id`] = tagId;
        ...
        if (updateEntries.length > 0) {
            const placeholders = updateEntries.map(([tagId, tagPath]: [number, string], i: number) => `WHEN $e${i}id THEN $e${i}path`).join(' ');
            q += `UPDATE tags SET path = CASE id ${placeholders} END WHERE id IN (${updateEntries.map(([tagId, tagPath]: [number, string], i: number) => `$e${i}id`).join(',')});`;
        }
        q += "COMMIT;";
        // console.log(q);
        try {
