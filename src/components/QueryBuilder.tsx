...
const operators = [
  ...
  { name: 'doesNotContain', value: 'doesNotContain', label: 'not contains' } as const,
  { name: 'beginsWith', value: 'beginsWith', label: 'starts with' } as const,
  { name: 'doesNotBeginWith', value: 'doesNotBeginWith', label: 'not starts with' } as const,
  { name: 'endsWith', value: 'endsWith', label: 'ends with' } as const,
  { name: 'doesNotEndWith', value: 'doesNotEndWith', label: 'not ends with' } as const,
...
export default () => {
  ...
  const toggleIcon = <IconButton onClick={toggle} color='secondary'><CloseIcon /></IconButton>;
  return !active ? null : (
    <Stack direction="row" sx={{ minWidth: "fit-content" }} pb={1} alignItems="flex-start" spacing={1}>
      {/* <Typography variant="h5">Filter Tags</Typography> */}
      <QueryBuilderMaterial>
        {toggleIcon}
        <Box flexGrow={1} >
          <QueryBuilder fields={fields} query={query} onQueryChange={handleQueryChange} operators={operators} />
        </Box>
      </QueryBuilderMaterial>
      {/* <h4>
