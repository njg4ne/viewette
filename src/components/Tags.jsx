...
export default function Tags({ tags: tagsSignal }) {
    ...
    const tagObjList = tagMapEntries.map(([tagId, tagStr]) => ({ tagId, tagStr }));
                ...
                <Tree
                    ...
                    render={(node, { depth, isOpen, onToggle, hasChild }) => {
                        ...
                        return (
                            ...
                            <Stack direction="row" sx={{ alignItems: "center", my: 0.5, ml: depth * 3 }} spacing={1}>
                                <IconButton size="small" sx={{ mr: 0.5 }} disableRipple={!hasChild} onClick={onToggle} >
                                    {isOpen ? <ExpandMoreIcon /> : <ChevronRightIcon />}
                                </IconButton>
                                <TagChip tag={node.text} kind={node.tagId ? "file" : "directory"} />
...
function TagTree({ data }) {
    ...
    return (
        <Box sx={{ minHeight: 110, flexGrow: 1, maxWidth: 300, alignSelf: "start" }}>
            <TreeView
...
function nest(tags) {
    ...
    const helper = tagObjArr => {
        ...
        nonNested = nonNested.map(({ tagId: id, tagStr: s }) => [s, `\$#${id}-${s}`]);
        nonNested = Object.fromEntries(nonNested);
        let groupedObj = group(tagObjArr);
        // replace the keys with the nested keys
        groupedObj = Object.fromEntries(Object.entries(groupedObj).map(([k, v]) => [nonNested[k] || k, v]));

        const entries = Object.entries(groupedObj);
        for (const [k, v] of entries) {
            const grouped = helper(v.map(o => ({ ...o, tagStr: parts(o.tagStr).slice(1).join(separator) })));
...
function nestForReactDndTreeview(tags) {
    ...
    const helper = tagObjArr => {
        ...
        for (const [k, v] of entries) {
            v.sort((a, b) => b.tagStr.split(separator).length - a.tagStr.split(separator).length);
            for (const { tagId, tagStr } of v) {
                ...
                const indexOfResult = stagedResult.findIndex(({ tag }) => tag === tagStr);
                if (indexOfResult !== -1) {
...
function dndDataFindChildren(data, id) {
    const helper = (data, parentId) => {
        const children = data.filter(({ parent: p }) => p === parentId).map(({ tagId, id }) => ({ tagId, id }));
        if (children.length === 0) return [];
        return children.reduce((acc, { id, tagId }) => [...acc, tagId, ...helper(data, id)], []).filter(x => x !== undefined)
    }
