...
export default function UploadDatabase() {
    async function onFormChange(f) {
        ...
        const exists = await root.getFileHandle(name).then(() => true).catch(() => false);
        let confirm = !exists;
        const phrase = "del";
        ...
        if (confirm) {
            ...
            file = new File([file], "database.sqlite3", { type: "application/x-sqlite3" });
            // save this file to opfs
            // let root = await navigator.storage.getDirectory();
            const dbFileHandle = await root.getFileHandle('database.sqlite3', { create: true });
    ...
    async function downloadDatabase() {
        ...
        const dbFileHandle = await root.getFileHandle('database.sqlite3', { create: false });
    ...
    return (<Box component="form" method="post" encType="multipart/form-data" sx={{ alignSelf: "center" }}>
        <Button sx={{ width: "100%" }} component="label" variant="contained" startIcon={<CloudUploadIcon />} htmlFor="upload-database" color='secondary' >
            Upload database
            <VisuallyHiddenInput type="file" accept=".sqlite3" id="upload-database" name="upload-database" onChange={e => { onFormChange(e.target.form) }} />
        </Button>

        <Button sx={{ width: "100%", marginTop: "1rem" }} variant="contained" color="secondary" onClick={clearOpfs} startIcon={<DeleteForeverIcon />}>
            Clear uploaded
        </Button>

        <Button sx={{ width: "100%", marginTop: "1rem" }} variant="contained" color="secondary" onClick={downloadDatabase} startIcon={<CloudDownloadIcon />}>
            Download database
        </Button>
